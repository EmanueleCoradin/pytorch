...
size_t parseChosenWorkspaceSize() {
  const char * val = getenv("TORCH_CUBLAS_WORKSPACE_CONFIG");
#ifdef USE_ROCM
  if (!val) {
  ...
  if (val) {
    ...
    if (next == end) {
      TORCH_WARN("Could not parse TORCH_CUBLAS_WORKSPACE_CONFIG, using default workspace size of ", default_size, " bytes.");
      return default_size;
    }
    while (next != end) {
      std::smatch match = *next;
      TORCH_CHECK(match.size() == 3, "Expected TORCH_CUBLAS_WORKSPACE_SPACE_CONFIG match of size 3 (Format :SIZE:COUNT)");
